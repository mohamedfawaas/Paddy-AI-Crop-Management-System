...
public class SuitabilityController {
    ...
    public SuitabilityController(MLClientService mlClient,
    ...
    public ResponseEntity<?> predict(@RequestBody Map<String, Object> payload,
        ...
        try {
            Map<String, Object> result = mlClient.predictSuitability(payload);
            Long predictionId = save(ud, "SUITABILITY", payload, result);
            // Feature 2: expose the saved prediction's id so the UI can submit feedback on it
    ...
    public ResponseEntity<?> history(@AuthenticationPrincipal UserDetails ud) {
        ...
        return ResponseEntity.ok(
                predictionRepo.findByUserAndTypeOrderByCreatedAtDesc(found.get(), "SUITABILITY"));
    }
