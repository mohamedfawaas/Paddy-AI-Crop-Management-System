...
const def = { temperature:'', humidity:'', rainfall_forecast_mm:'', growth_stage:'Vegetative' }
const STAGES = ['Land Preparation', 'Seedling', 'Vegetative', 'Tillering', 'Flowering', 'Ripening']
...
export default function WeatherPage() {
  ...
  const submit = async e => {
    ...
    try {
      const data = await getWeatherAdvisory({
      ...
      setResult(data)
      toast.success(`✅ ${t('advisoryReadyMsg')}`)
    ...
    <div>
      <div className="page-header">
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
          <CloudSun size={16} color="var(--green)"/>
          <span style={{ fontSize:11, fontFamily:'monospace', letterSpacing:3, color:'var(--green)', textTransform:'uppercase' }}>{t('weather')}</span>
        </div>
      ...
      <div className="grid-2" style={{ alignItems:'start' }}>
          ...
          <form onSubmit={submit}>
            <div className="grid-2">
              ...
              <div className="input-group">
                ...
                <input className="input-field" name="humidity" type="number" step="0.1" placeholder="e.g. 75" value={form.humidity} onChange={handle} required/>
              </div>
              <div className="input-group">
                <label>{t('rainfallForecast')}</label>
                <input className="input-field" name="rainfall_forecast_mm" type="number" step="1" placeholder="e.g. 20" value={form.rainfall_forecast_mm} onChange={handle} required/>
              </div>
              <div className="input-group">
        ...
        <div>
          {result
            ? <ResultDisplay result={result} type="weather" t={t}/>
            : <div className="card" style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:280, textAlign:'center', gap:14 }}>
                ...
                <div style={{ fontSize:14, color:'rgba(255,255,255,0.3)' }}>
                  {t('fillForm')}<br/><strong style={{color:'var(--green)'}}>{t('getWeatherAdvisory')}</strong>
                </div>
              </div>
          }
        </div>
