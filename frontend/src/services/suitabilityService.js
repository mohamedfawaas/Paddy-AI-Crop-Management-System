import api from './api'
export const predictSuitability = async (payload) => {
  try {
    const res = await api.post('/suitability/predict', payload)
    return res.data
  } catch (err) {
...
export const getSuitabilityHistory = async () => {
  try { const res = await api.get('/suitability/history'); return res.data || [] }
  catch { return [] }
}
